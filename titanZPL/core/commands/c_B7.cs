...
namespace titanZPL.commands  {
    public class zpl_cmd_c_B7 : zpl_command{   //PDF417 Bar Code
        ...
        public    int number_of_data_columns_to_encode                             = 0;
        public    int number_of_rows_to_encode                                     = 0;
        public string truncate                                                     = String.Empty;
                                        
        public zpl_cmd_c_B7(string data,List<zpl_command> commands):base(commands){                    
            ...
            bar_code_height_for_individual_rows                         =(   int)argument(1,data,"i","1-32000"  ,"","10",by.bar_code_height.ToString());
            security_level                                              =(   int)argument(2,data,"i","0-8"      ,"","0","");
            number_of_data_columns_to_encode                            =(   int)argument(3,data,"i","1-30"     ,"","1","");
            number_of_rows_to_encode                                    =(   int)argument(4,data,"i","3-90"     ,"","-1","");
            truncate                                                    =(string)argument(5,data,"s","Y,N"      ,"","N","");
...
h =bar_code_height_for_individual_rows //1 to height of label  //value set by ^BY This number multiplied by the module equals the height of the individual rows in dots. If this number is not specified, the overall bar code height, divided by the number of rows, equals the height of the individual rows in dots, where the overall bar code height is defined by the ^BY command. 1 is not a recommended value. 
	
s =security_level //1 to 8 (error detection and correction)  //0 (error detection only) This determines the number of error detection and correction code-words to be generated for the symbol. The default level provides only error detection without correction. Increasing the security level adds increasing levels of error correction and increases the symbol size. 
	
c =number_of_data_columns_to_encode //1 to 30  //1:2 (row-to-column aspect ratio) You can specify the number of code-word columns giving control over the width of the symbol. 
	
r =number_of_rows_to_encode //3 to 90  //1:2 (row-to-column aspect ratio) You can specify the number of symbol rows giving control over the height of the symbol. For example, with no row or column values entered, 72 code-words would be encoded into a symbol of six columns and 12 rows. Depending on code-words, the aspect ratio is not always exact.  59 ZPL Commands ^B7 11/21/16 Zebra Programming Guide P1012728-011 
	
t =truncate_right_row_indicators_and_stop_pattern //
N = no truncation Y = perform truncation  //N Parameters Detail
  ...
  **************************************************/ 
			...
			"overall bar code height is defined by the "+ 
			"^BY command. 1 is not a "+ 
			"recommended value. "+ 
			"s = security level "+ 
			...
			"r = ratio "+ 
			"Fixed Value: "+ 
			"3 (ratio has no effect on PDF417) "+ 
			"h "+ 
			...
			"^FD statements. This scheme is used: "+ 
         ...
         public override void draw() {
	        ...
	        bc.RegistrationName=bytescout_registrationName;
            bc.RegistrationKey =bytescout_registrationKey;

            bc.Options.PDF417MinimumColumnCount =number_of_data_columns_to_encode;
            if(-1!=number_of_rows_to_encode) {
                bc.Options.PDF417RowCount=number_of_rows_to_encode;
            }
            
            bc.Options.PDF417ErrorCorrectionLevel=(PDF417ErrorCorrectionLevel) security_level;
            bc.BarHeight=bar_code_height_for_individual_rows;
            bc.Symbology=SymbologyType.PDF417;
            bc.Value = _internal.field_data;
            bc.GetImage();
            
/*
             bar_code_height_for_individual_rows                         
            security_level                                              
            number_of_data_columns_to_encode                            
            number_of_rows_to_encode                                    
  */    
    
            _internal.image=(Bitmap)bc.GetImage();
            _internal.baseline=bar_code_height_for_individual_rows;
